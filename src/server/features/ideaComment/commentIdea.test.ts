...
describe("お題のコメントAPI", () => {
  it("お題にコメントできる", async () => {
    ...
    await caller.ideaComment.comment({ ideaId: idea.id, comment });
  ...
  it("お題に返信できる", async () => {
    ...
    const { idea } = await TestHelpers.createIdeaAndUser();
    const { commentId: parentCommentId } = await caller.ideaComment.comment({
      ideaId: idea.id,
      comment: "first",
    });

    await caller.ideaComment.comment({
      ideaId: idea.id,
      comment: "comment",
  ...
  describe("バリデーションが失敗する入力", () => {
    it.each([
      ...
      const promise = caller.ideaComment.comment({ ideaId: idea.id, comment });
